...
ビルド後に `npm link` を実行すると、ローカルのソースコードをグローバルコマンドとして登録できます。

```bash
npm install          # 依存パッケージのインストール（自動でビルドも実行）
npm link             # `md-dive` コマンドをグローバルに登録

md-dive outline README.md   # 動作確認
```

確認が終わったら `npm unlink` でグローバル登録を解除できます。

```bash
npm unlink
```

`npm link` を使わず直接実行することもできます。

```bash
node dist/cli.js outline README.md
```
